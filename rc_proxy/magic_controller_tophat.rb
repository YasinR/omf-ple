...
module OmfRc::ResourceProxy::Magic
  ...
  request :slice do |resource|
    ...
    ret.instance_variable_get(:@http).instance_variable_set(:@verify_mode, OpenSSL::SSL::VERIFY_NONE)

    # say params passed in is { slice_hrn: "ple.upmc.myslicedemo" }
    #key = params.keys.first
    pairwises = ret.call("Get", auth, "pairwise", "latest", {}, ['agent_src'])
 	for pairwise in pairwises[0..1]
		puts pairwise
	end
  end
